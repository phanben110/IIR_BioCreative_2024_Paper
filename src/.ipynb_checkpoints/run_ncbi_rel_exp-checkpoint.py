...
class DatasetProcessor(object):
    ...
    def _get_dataset(self, 
        ...
        try:
            if has_header:
                data_df = pd.read_csv(data_file, sep='\t', dtype=str, keep_default_na=False).fillna(np.str_(''))
            else:
                data_df = pd.read_csv(data_file, sep='\t', header=None, dtype=str, keep_default_na=False).fillna(np.str_(''))
            ...
            if self.no_neg_for_train_dev:
                ...
                for i, label in enumerate(data_dict[self.label_name]):
                    if label not in neg_labels:
                        subset.append(i)
                print("Data dict", data_dict[self.label_name])
        ...
        print(len(self.transformed_ds['train']['label']))
        int = 0
        for label in self.transformed_ds['train']['label']:
            if label == '':
                int +=1
        print("None value: ", int)
        if set_type == 'train':    
            data_ds = (
